...
def bfs(frontier: deque, explored_set: list, pathways: dict, goal: Callable[[int], bool]):
    """Performs a breadth-first search for a goal state.
    
    Args:
    frontier: The queue of states to explore (deque).
    explored_set: The list of explored states (list).
    pathways: A dictionary representing pathways between states (dict).
    ...
    while not resolved:
        ...
        print(f"The following states have now been explored: {explored_set}", end="\n\n")
        print(f"The new frontier is comprised of states {frontier}", end="\n\n\n")
